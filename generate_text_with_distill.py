...
def call_llama2(config_file,prompts, max_input_tokens = 512,max_output_tokens = 1536, watermark_name = str, model_name = str):
    config = load_file(config_file)
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    tokenizer.pad_token = tokenizer.eos_token
    model = AutoModelForCausalLM.from_pretrained(model_name, device_map = 'auto')
    model.eval()
    output_path = f"{config.output_path}/distill/{watermark_name}.json"
    output_generations = []
    id = 0
...
def generate_watermarktext(config_file, watermarks=None, generate_text_path = str,watermark_name = str, model_name = str):
    config_class= load_file(config_file)
    prompts = read_prompt(config_class.prompt_path)
    output_path = call_llama2(config_file,prompts, watermark_name=watermark_name, model_name = model_name)
    pass
    ...
    for watermark in tqdm(watermarks,desc = "Processing Watermarks"):
        ...
        for generation in tqdm(generations,desc = f"Processing {watermark.name} watermarks:"):
            ...
            if watermark_name == "Format":
                ...
                input = generation.response
                instance = LexicalWatermark(config=config_class, tau_word=0.75, tau_sent=0.8, lamda=0.5)
                output = instance.injection(str(input))
...
if __name__ == "__main__":
    ...
    watermark_name = str(sys.argv[3])
    model_name = str(sys.argv[4])
    generate_watermarktext(config_file=config_path, generate_text_path=output_path, watermark_name=watermark_name, model_name=model_name)
