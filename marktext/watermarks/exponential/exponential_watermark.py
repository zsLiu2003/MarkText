...
from exponential.randomness import generate, generate_rnd
...
from exponential.exponential_base import exponential_edit_score,exponential_key_func,exponential_sampling,exponential_score
...
class Exponential():
    ...
    def __init__(self, config: ConfigSpec):
        ...
        self.watermark_name = "Exponential"
    ...
    def injection_detection(
        ...
        generate_watermark = lambda prompt,seed : generate(
            ...
            seed=seed,
            key_func=exponential_key_func,
            sampler=exponential_sampling,
            random_offset=True
            
        )
        dist = lambda x,y : exponential_edit_score(x,y, gamma=gamma)
        ...
        for idx,prompt in tqdm(enumerate(encode_prompts), desc="Processing exponential watermark injection-----"):
            watermark_token = generate_watermark(prompt,seed=seeds[idx])[:,512:]
            encode_watermark_tokens.append(watermark_token)
        # for idx, generation in tqdm(enumerate(generations), desc="Processing encodeing---------"):
        ...
        for idx, watermark in tqdm(enumerate(encode_watermark_tokens), desc="Processing decoding------"):
            ...
            generation = Generation(id=idx, prompt=prompts[idx],response=output_text,watermark_name="Convert", attack="Emoji")
        ...
        #     watermark_data = {
        #         "name": f"{self.watermark_name}_{attack_name}",
        #         "watermark_percent": watermark_percent
        #     }
        #     dict_results.append(dict_data)
        #     watermark_percent_results.append(watermark_data)
        # output_path = f"{config.output_path}/watermark/{self.watermark_name}/quality/is_watermark_list2.json"
        # with open(output_path, "w") as outputfile:
        #     json.dump(dict_results,outputfile,indent=4)
        
        # output_watermark_path = f"{config.output_path}/watermark/{self.watermark_name}/quality/watermark_percent2.json"
